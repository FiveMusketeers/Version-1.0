...
-(void)LoadPictures{
    ...
    while (UpperBound >= PicturePosition)
    {
        ...
        if (!path){
             NSLog(@"The path for the item is: %@", path);
            NSString* fullPath = [self FindPath:itemRead.imagePath];
            NSLog(@"The new path for the item is: %@", fullPath);
...
- (IBAction)select:(OurButton*)buttonSelected;
{
    ...
    {
        NSString *nameSelected = buttonSelected.getImageName;
        NSString *fileSelected = buttonSelected.getImageFile;
        
        ListItem *itemSelected = [[ListItem alloc] initWithName:nameSelected imagePath:fileSelected];
    
        if (isCreating){
            NSLog(@"We are adding an item with name %@, and path %@", itemSelected.name, itemSelected.imagePath);
